...
import { TensePracticeHub, PracticeTab } from "@/components/grammar/tenses/tense-practice-hub";
...
export default function SimplePresentTensePage() {
  ...
  const topicData = TENSES_DATA["simple-present"];

  const progressInfo = tenseProgressList.find((p) => p.categorySlug === "simple-present");
  ...
  const handleCompleteLesson = async () => {
    await recordLessonComplete("simple-present", "completed");
  ...
  return (
    <div className="space-y-8 animate-in fade-in-50 duration-300 max-w-5xl mx-auto">
      ...
      {currentMode === "test" && (
        <TenseTestEngine
          key={`test-sp-${retakeKey}`}
          topicName={topicData.name}
          categorySlug="simple-present"
          questions={topicData.testQuestions}
          onSubmitTest={async (score, total, incorrect) => {
            return await submitTestResult("simple-present", score, total, incorrect);
          }}
